/*
 * 版权声明 (Copyright 2015 苏黎世联邦理工学院 等)
 * 根据 Apache License 2.0 授权使用
 * 除非符合许可证，否则不得使用此文件
 */

// 避免头文件重复包含的预处理器指令（原文件可能未显式使用 #pragma once，但通常有此保护）
// #pragma once 或 #ifndef 风格的保护在原始文件中没有，这里仅说明
...
class GazeboMotorModel : public MotorModel, public ModelPlugin {
 public:
  ...
  GazeboMotorModel()
      : ModelPlugin(),
        MotorModel(),
        command_sub_topic_(kDefaultCommandSubTopic),       // 命令话题
  ...
  virtual ~GazeboMotorModel(); // 虚析构函数
